...
from PyQt5.QtWidgets import QApplication, QWidget, QComboBox, QVBoxLayout, QPushButton, QListWidget, QProgressBar, \
    QSplitter
import PyQt5
import os, re
...
def scan_button():
    ...
    def do_scan():
        file_name = file_list(selected_drive)
        update_gui(file_name)

    thread = threading.Thread(target=do_scan)
    thread.start()


def update_gui(file_name):
    count = 0
    for trans_one in reversed(file_name):
        for trans_two in trans_one:
            if count / 2 == 0:
                ...
                trans_two = (int(trans_two) / (1024 ** 3))
    ...
    # Finalize progress bar once complete
    progress_bar.setValue(lennn)
...
# Connect scrollbars of 'path' and 'size'
